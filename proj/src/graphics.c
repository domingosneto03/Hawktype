...
int (draw_pixel)(uint16_t x,uint16_t y,uint32_t color){
    ...
    if (x > cur_mode_info.XResolution || y > cur_mode_info.YResolution) return 1;
    ...
    if (memcpy(&secondary_frame_buffer[pixel_index], &norm_color, bpp) == NULL) return 1;
    return 0;
}
...
int (draw_rectangle)(uint16_t x,uint16_t y, uint16_t width, uint16_t height, uint32_t color){
    for (int i = 0; i < height; i++) {
        if (draw_horizontal_line(x, y+i, width, color)!=0) return 1;
    }
    return 0;
}
...
int (direct_mode)(int j, int i, uint32_t first, uint8_t step,uint32_t *color){
    uint32_t R,G,B;
    R = ((((1 << cur_mode_info.RedMaskSize) - 1) & (first >> cur_mode_info.RedFieldPosition)) + j * step) % (1 << cur_mode_info.RedMaskSize);
    G = ((((1 << cur_mode_info.GreenMaskSize) - 1) & (first >> cur_mode_info.GreenFieldPosition)) + i * step) % (1 << cur_mode_info.GreenMaskSize);
    B = ((((1 << cur_mode_info.BlueMaskSize) - 1) & (first >> cur_mode_info.BlueFieldPosition)) + (i + j) * step) % (1 << cur_mode_info.BlueMaskSize);
    *color = (R << cur_mode_info.RedFieldPosition | G << cur_mode_info.GreenFieldPosition | B << cur_mode_info.BlueFieldPosition);
    return 0;
}
...
int draw_static_background() {
    ...
    draw_xpm_sentence("type", box_x - 80, box_y + 4, "default");
